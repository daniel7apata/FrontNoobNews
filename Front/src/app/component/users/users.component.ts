...
export class UsersComponent implements OnInit {
  ...
  redactor: LogicUser = new LogicUser();
  mensaje: string = '';
  ...
  ngOnInit(): void {
    ...
    this.pubS.list().subscribe((data) => {
      let filteredData1 = data.filter((publication) => publication.logicUser.id === this.id);
    ...
    this.ackS.list().subscribe((data) => {
      let filteredData2 = data.filter((acknowledgment) => acknowledgment.logicUser.id === this.id);
    ...
    this.extS.list().subscribe((data) => {
      let filteredData3 = data.filter((externalPublication) => externalPublication.logicUser.id === this.id);
